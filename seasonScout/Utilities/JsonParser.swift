...
struct JsonParser {
        ...
        do {
            ...
            for rawProduct in rawProducts! {
                ...
                let storageInstructions =
                    rawProduct["storageInstructions"] as! String
                let energyConsumption =
                    rawProduct["energyConsumption"] as! String
                ...
                let seasonalDataRaw =
                    rawProduct["seasonalData"] as! [String: [String]]
                ...
                guard let type = ProductType(rawValue: typeString) else {
                    print(
                        "Warning: Unknown product type '\(typeString)' found in JSON."
                    )
                    continue  // Skip this product if type is unknown
                }
                guard let subtype = ProductSubtype(rawValue: subtypeString)
                else {
                    print(
                        "Warning: Unknown product subtype '\(subtypeString)' found in JSON."
                    )
                    continue  // Skip this product if subtype is unknown
                }

                // Convert seasonal data from dictionary to sorted tuple array
                let seasonalDataTuples = seasonalDataRaw.compactMap {
                    (monthString, availabilityStrings) -> (Month, [String])? in
                    if let month = Month(rawValue: monthString) {
                        return (month, availabilityStrings)
                    } else {
                        print(
                            "Warning: Unknown month string '\(monthString)' found in JSON."
                        )
                        return nil
                    }
                }
                ...
                let sortedSeasonalDataTuples = seasonalDataTuples.sorted {
                    guard let firstIndex = orderedMonths.firstIndex(of: $0.0),
                        let secondIndex = orderedMonths.firstIndex(of: $1.0)
                    else {
                ...
                for (month, availabilityStrings) in sortedSeasonalDataTuples {
                    let availability = availabilityStrings.compactMap {
                        Availability(rawValue: $0)
                    }
                    seasonalData.append(
                        contentsOf: availability.map {
                            SeasonalData(month: month, availability: $0)
                        })
                }
    ...
    static func parseToRecipes(fileName: String) -> [Recipe] {
        guard let path = Bundle.main.path(forResource: fileName, ofType: "json")
            ...
            print(
                "Error: File \(fileName) not found, returning an empty recipe list."
            )
            return []
        }
        ...
        do {
            ...
            for item in jsonArray {
                ...
                if let id = item["id"] as? Int,
                    ...
                    let seasonalDataRaw = item["seasonalData"]
                        as? [String: String],
                    let ingredientsByPersonsRaw = item["ingredientsByPersons"]
                        as? [String: [String: String]]
                {
                    // Convert String to Enum
                    guard let category = RecipeCategory(rawValue: categoryRaw),
                        let effort = Level(rawValue: effortRaw),
                        let price = Level(rawValue: priceRaw)
                    else {
                        print(
                            "Error: Invalid category, effort, or price for recipe '\(title)', (ID: \(id)). Skipping this recipe."
                        )
                        continue
                    }
                    ...
                    let sortedSeasonalDataRaw = seasonalDataRaw.keys.sorted {
                        (firstKey, secondKey) in
                        guard
                            let firstIndex = orderedMonths.firstIndex(
                                of: Month(rawValue: firstKey) ?? .jan),
                            let secondIndex = orderedMonths.firstIndex(
                                of: Month(rawValue: secondKey) ?? .jan)
                        else {
                    ...
                    for monthString in sortedSeasonalDataRaw {
                        if let month = Month(rawValue: monthString) {
                            let availability =
                                seasonalDataRaw[monthString] ?? ""
                            seasonalData.append(
                                RecipeSeasonalMonthData(
                                    month: month, availability: availability))
                        } else {
                            print(
                                "Warning: Invalid month \(monthString) in seasonal data for recipe \(title). Skipping."
                            )
                        }
                    }
                    ...
                    for (key, ingredients) in ingredientsByPersonsRaw {
                        if let personCount = Int(key) {
                            var ingredientList: [Ingredient] = ingredients.map {
                                Ingredient(name: $0.key, amount: $0.value)
                            }

                            // Sort the ingredientList alphabetically by ingredient name
                            ingredientList.sort { $0.name < $1.name }

                            ingredientsByPersons.append(
                                PersonsIngredients(
                                    personNumber: personCount,
                                    ingredients: ingredientList))
                        } else {
                            print(
                                "Warning: Invalid person count \(key) in ingredients for recipe \(title). Skipping."
                            )
                        }
                    }
                    ...
                    print(
                        "Error: Missing or invalid data for recipe \(item["title"] ?? "Unknown Title"). Skipping this recipe."
                    )
                }
            }
