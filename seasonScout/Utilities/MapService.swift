...
struct MapService {
    ...
    static func getLookAroundScene(mapItem: MKMapItem) async
        -> MKLookAroundScene?
    {
    ...
    static func getRouteAndETA(
        userCoordinate: CLLocationCoordinate2D, mapItem: MKMapItem,
        transportType: MKDirectionsTransportType
    ) async -> RouteData? {

        let routeRequest = MKDirections.Request()
        routeRequest.source = MKMapItem(
        ...
        do {
            ...
            if transportType == .transit {
                let etaResponse = try await MKDirections(request: routeRequest)
                    .calculateETA()
                ...
                let response = try await MKDirections(request: routeRequest)
                    .calculate()
