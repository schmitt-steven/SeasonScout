...
class MonthSwitcherService {
    ...
    static func changeMonth(
        selectedMonth: Binding<Month>, direction: SwipeDirection
    ) {
        let months = Month.allCases  // Get all months as an array
        guard
            let currentIndex = months.firstIndex(of: selectedMonth.wrappedValue)
        else { return }  // Find the current month's index
